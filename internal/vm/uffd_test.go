...
func TestBitsetSetGetCount(t *testing.T) {
	b := newBitset(200)
	for _, i := range []uint64{0, 1, 63, 64, 65, 199} {
		b.set(i)
	}
	b.set(200) // out of range — ignored, no panic
	b.set(999)
	for _, i := range []uint64{0, 1, 63, 64, 65, 199} {
		if !b.get(i) {
	...
	for _, i := range []uint64{2, 62, 66, 198, 200, 999} {
		if b.get(i) {
...
func TestBitsetRuns(t *testing.T) {
	b := newBitset(100)
	// two runs: [3,5) and [10,13)
	for _, i := range []uint64{3, 4, 10, 11, 12} {
		b.set(i)
	}
	runs := b.runs()
	want := []pageRun{{3, 2}, {10, 3}}
...
func TestBitsetMarshalRoundtrip(t *testing.T) {
	b := newBitset(500)
	for _, i := range []uint64{0, 100, 256, 499} {
		b.set(i)
	}
	got := bitsetFromBytes(b.marshal(), 500)
	if got.count() != b.count() {
		t.Fatalf("count after roundtrip = %d, want %d", got.count(), b.count())
	}
	for _, i := range []uint64{0, 100, 256, 499} {
		if !got.get(i) {
			t.Errorf("bit %d lost in roundtrip", i)
		}
	}
	// A short buffer must clamp, not panic.
	_ = bitsetFromBytes([]byte{0xff}, 500)
}

func TestHostAddrForOffset(t *testing.T) {
	regions := []guestRegion{
		{BaseHostVirtAddr: 0x1000_0000, Size: 0x10000, Offset: 0},
		{BaseHostVirtAddr: 0x9000_0000, Size: 0x10000, Offset: 0x10000},
	}
	// region 0: offset 0x1234 → base+0x1234
	if dst, ok := hostAddrForOffset(regions, 0x1234); !ok || dst != 0x1000_1234 {
		t.Errorf("dst=%#x ok=%v, want 0x10001234", dst, ok)
	}
	// region 1: offset 0x10500 → base + (0x10500-0x10000)
	if dst, ok := hostAddrForOffset(regions, 0x10500); !ok || dst != 0x9000_0500 {
		t.Errorf("dst=%#x ok=%v, want 0x90000500", dst, ok)
	}
	// offset in no region's file range
	if _, ok := hostAddrForOffset(regions, 0x50000); ok {
		t.Error("expected no mapping for gap offset")
	}
}
