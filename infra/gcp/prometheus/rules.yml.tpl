...
# so that used slots PLUS queued creates PLUS a headroom buffer fit. Queued
# creates (the gateway's bounded create queue) are demand that found no slot —
# counting them makes a burst larger than the headroom pull scale-up
  ...
  - name: sandbox
    ...
    rules:
      - record: sandbox:workers_desired
        expr: clamp_min(ceil((sandbox_slots_used + sandbox_create_queue_depth + ${HEADROOM_SLOTS}) / ${SLOTS_PER_HOST}), 1)
